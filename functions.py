...
def V_theta_t_plus1(
    ...
    d_ln_S1t, d_ln_S2t, d_ln_S3t, d_ln_Lt,
    ln_Lt_plus1, c1,
    weights, input_scaler, output_scaler, train_V=True):
    ...
    out_scaled = merged @ w_out + b_out
    value_function_scaled = out_scaled[:, 0:1] 

    return value_function_scaled if train_V else output_scaler.inverse_transform(value_function_scaled)
...
def V_t(
        ...
        ln_Lt, C_t, 
        u, mu, Phi1, Phi2, p,
        NN_weights, inputscaler, outputscaler, quantizer, t, 
        weights, gamma, v, train_V=1
    ):
...
def optimize_u_general(
    ...
    # Step 1: optimize u1 and u2
    def expected_negative_return(u12):
        u1, u2 = u12
        ...
        u3 = 1 - u1 - u2
        weighted_sum = u1 * np.exp(d1) + u2 * np.exp(d2) + u3 * np.exp(d3)
        return -np.sum(quantize_weights * weighted_sum)

    res_u12 = minimize(
        expected_negative_return,
        x0=best_u12,
        method='SLSQP',
...
def IndividualTest(c0, #gamma, 
    ...
    for t in range(0, T):
        ...
        if t < T-1:
            out_scaler = out_scaler_valuefun[t+1]
            def g_i(u): 
                V = V_t(
                        ...
                        u, mu,  Phi1, Phi2, p, 
                        V_hat_theta[t+1].get_weights(), input_scaler_valuefun, out_scaler, quantizer, t,
                        weights, gamma, v
                        )
                return V *-1
        else:
        ...
        samples[0][4][t+1] = \
            ...
            (samples[0][4][t] +p - samples[0][4][t]*samples[0][0][t]) - ln_L_t_plus1 
        samples[0][5][t] = samples[0][4][t]*samples[0][0][t] # index 5 means dividend payout
        # samples[0][6][t] = -1* result.fun

        
        samples[1][1][t] = 1-0.8393392057419106-0.1210072070468903 
        samples[1][2][t] = 0.8393392057419106 
        samples[1][3][t] = 0.1210072070468903
        samples[1][4][t+1] = \
            (samples[1][1][t] * R1_t_plus1 +
             samples[1][2][t] * R2_t_plus1 +
             samples[1][3][t] * R3_t_plus1) *\
            (samples[1][4][t] +p - samples[1][4][t]*samples[0][0][t]) - ln_L_t_plus1 
        samples[1][5][t] = samples[1][4][t]*samples[1][0][t]
                    

        samples[2][1][t] = 1-0.5778736759513535-0.3491565319733228
        samples[2][2][t] = 0.5778736759513535 
        samples[2][3][t] = 0.3491565319733228 
        samples[2][4][t+1] = \
            (samples[2][1][t] * R1_t_plus1 +
             samples[2][2][t] * R2_t_plus1 +
             samples[2][3][t] * R3_t_plus1) *\
            (samples[2][4][t] +p - samples[2][4][t]*samples[0][0][t]) -  ln_L_t_plus1 
        ...
        samples[3][4][t+1] = \
            ...
            (samples[3][4][t] +p - samples[3][4][t]*samples[0][0][t]) -  ln_L_t_plus1 
            

        samples[4][1][t] = 1-0.7635329368640394-0.1896157492610563
        samples[4][2][t] = 0.7635329368640394
        samples[4][3][t] = 0.1896157492610563
        samples[4][4][t+1] = \
            (samples[4][1][t] * R1_t_plus1 +
             samples[4][2][t] * R2_t_plus1 +
             samples[4][3][t] * R3_t_plus1) *\
            (samples[4][4][t] +p - samples[4][4][t]*samples[0][0][t]) -  ln_L_t_plus1 

        # # V_T optimization strategy here
        # def obj(u): 
        #           cap, div = forward(samples[5][4][t], u, 
        ...
        chosen_strategy = random.randint(1, 4)
        samples[5][1][t] = samples[chosen_strategy][1][t]
        samples[5][2][t] = samples[chosen_strategy][2][t]
        samples[5][3][t] = samples[chosen_strategy][3][t]
        samples[5][4][t+1] = \
            (samples[5][1][t] * R1_t_plus1 +
              samples[5][2][t] * R2_t_plus1 +
              samples[5][3][t] * R3_t_plus1) *\
            (samples[5][4][t] +p - samples[5][4][t]*samples[0][0][t]) - ln_L_t_plus1 
...
def RunTests(
             ...
             div_lo, div_up, stock_lo, stock_up, cbond_lo, cbond_up, 
             input_scaler, out_scaler_valuefun, V_hat_theta,
             numTest
            ):    
